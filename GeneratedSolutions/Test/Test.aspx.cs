...
namespace Web_TPMTrakDashboard
{
    public partial class Test : System.Web.UI.Page
    {
                ...
                foreach (string id in new[] { "Ddl1", "Ddl2", "Ddl3", "Ddl4", "Ddl5" }) {
                    ...
                    if (ctrl is ListControl lc) {
                        ...
                        if (dt != null) {
                            ...
                            if (lc is DropDownList) lc.Items.Insert(0, new ListItem("-- Select --", ""));
                        }
        ...
        private void BindFilterControl(ListControl lc, string id, string name, bool d, string p1 = null, string p2 = null)
        {
            ...
            lc.DataBind();
            if (d && lc is DropDownList) lc.Items.Insert(0, new ListItem("-- Select --", ""));
        ...
        private void CreateDynamicColumns()
        {
            if (gvReport.Columns.Count == 0)
            {
                ...
                boundCol.HeaderText = "Module";
                boundCol.DataField = "Module";
                gvReport.Columns.Add(boundCol);
                var boundCol = new BoundField();
                boundCol.HeaderText = "Machine";
                boundCol.DataField = "Machine";
                gvReport.Columns.Add(boundCol);
                var templateCol = new TemplateField();
                templateCol.HeaderText = "Status";
                templateCol.ItemTemplate = new DynamicTemplate("Status", "Checkbox");
                gvReport.Columns.Add(templateCol);
                var templateCol = new TemplateField();
                templateCol.HeaderText = "Value";
                templateCol.ItemTemplate = new DynamicTemplate("Value", "TextField");
                gvReport.Columns.Add(templateCol);
        ...
        {
            int colCount = 0; foreach(DataControlField col in gvReport.Columns) { if(col.Visible && col.HeaderText != "Action") colCount++; }
            Document pdfDoc = new Document(PageSize.A4.Rotate(), 10f, 10f, 10f, 0f);
            using (MemoryStream ms = new MemoryStream())
            {
                PdfWriter writer = PdfWriter.GetInstance(pdfDoc, ms); pdfDoc.Open();
                PdfPTable table = new PdfPTable(colCount); table.WidthPercentage = 100;
                PdfPCell headerCell = new PdfPCell(new Phrase(title + " REPORT", FontFactory.GetFont("Arial", 16, Font.BOLD)));
                headerCell.Colspan = colCount; headerCell.HorizontalAlignment = Element.ALIGN_CENTER; headerCell.Border = Rectangle.NO_BORDER; table.AddCell(headerCell);
                foreach (DataControlField col in gvReport.Columns) { if (col.Visible && col.HeaderText != "Action") { table.AddCell(new Phrase(col.HeaderText, FontFactory.GetFont("Arial", 10, Font.BOLD))); } }
    ...
    public class DynamicTemplate : ITemplate
    {
        ...
        public DynamicTemplate(string colName, string type) { _colName = colName; _type = type; }
