...
{
    public partial class BreakDownbySubsystem : System.Web.UI.Page
    {
        ...
        private void BindDownIdInfo()
        {
            try
            {
                var DownId = TMPTrakDataBase.GetBreakDownIdInfo("Subsystem", "");
        ...
        private async Task GetAsync()
        {
            try
            {
                ...
                else
                {
                    ...
                    var vardownTime = TMPTrakDataBase.GetBreakDownTimeAndFreqData(fromDate, toDate, machineSelection, plantId, downId, Exclude, "DTimeCategory", cellId, "ByCategory");
                    var vardownTimeFreq = TMPTrakDataBase.GetBreakDownTimeAndFreqData(fromDate, toDate, machineSelection, plantId, downId, Exclude, "DFreqCategory", cellId, "ByCategory");
                    ...
                    if (view != null && view.Table.Rows.Count > 0)
                        headerValues = view.ToTable(true, "machineDescription");
                    ...
                    {
                        string strValue = headerValues.Rows[i]["machineDescription"].ToString();
                    ...
                    {
                        if (downTimeWise.Rows[i]["BreakDownCatagory"].ToString() != downCode)
                        {

                            row = Values.NewRow();
                            Values.Rows.Add(row);
                            Values.Rows[Values.Rows.Count - 1]["DownCode"] = downTimeWise.Rows[i]["BreakDownCatagory"];
                        }
                        if (Convert.ToInt32(downTimeWise.Rows[i]["BreakDownCategoryTime"]) > 0)
                        {
                            t = TimeSpan.FromSeconds(Convert.ToInt32(downTimeWise.Rows[i]["BreakDownCategoryTime"]));
                            int seconds = Convert.ToInt32(downTimeWise.Rows[i]["BreakDownCategoryTime"]);
                            string h = string.Format("{0:00}:{1:00}:{2:00}", seconds / 3600, (seconds / 60) % 60, seconds % 60);
                            answer = string.Format("{0:00}:{1:00}:{2:00}",
                               (int)t.TotalHours,
                                     t.Minutes,
                                     t.Seconds);
                            Values.Rows[Values.Rows.Count - 1][downTimeWise.Rows[i]["machineDescription"].ToString()] = answer;
                        }
                        else
                            Values.Rows[Values.Rows.Count - 1][downTimeWise.Rows[i]["machineDescription"].ToString()] = string.Empty;
                        ...
                        downCode = downTimeWise.Rows[i]["BreakDownCatagory"].ToString();
                        if (dicValue.ContainsKey(downTimeWise.Rows[i]["machineDescription"].ToString()))
                            dicValue[downTimeWise.Rows[i]["machineDescription"].ToString()] = Convert.ToInt32(downTimeWise.Rows[i]["TotalMachine"]);
                        else
                            dicValue.Add(downTimeWise.Rows[i]["machineDescription"].ToString(), Convert.ToInt32(downTimeWise.Rows[i]["TotalMachine"]));
                    }
                    totalDown = 0;
                    ...
                    for (int i = 0; i < downTimeWise.Rows.Count; i++)
                    {
                        ...
                        Values.Rows[Values.Rows.Count - 1][downTimeWise.Rows[i]["machineDescription"].ToString()] = downTimeWise.Rows[i]["BreakDownCategoryFreq"];
                        downCode = downTimeWise.Rows[i]["BreakDownCatagory"].ToString();
                        //totalDown += Convert.ToInt32(downTimeWise.Rows[i]["DownFreq"]);
                        Values.Rows[Values.Rows.Count - 1]["Total"] = downTimeWise.Rows[i]["TotalBreakDownCategoryFreq"];

                        if (dicValue.ContainsKey(downTimeWise.Rows[i]["machineDescription"].ToString()))
                            dicValue[downTimeWise.Rows[i]["machineDescription"].ToString()] = Convert.ToInt32(downTimeWise.Rows[i]["TotalMachineFreq"]);
                        else
                            dicValue.Add(downTimeWise.Rows[i]["machineDescription"].ToString(), Convert.ToInt32(downTimeWise.Rows[i]["TotalMachineFreq"]));
                    }
                    ...
                    {
                        if (downTimeFreq.Rows[i]["BreakDownCatagory"].ToString() != downCode)
                        {
                            row = Values.NewRow();
                            Values.Rows.Add(row);
                            Values.Rows[Values.Rows.Count - 1]["DownCode"] = downTimeFreq.Rows[i]["BreakDownCatagory"];
                            //totalDown = 0;
                        }
                        Values.Rows[Values.Rows.Count - 1][downTimeFreq.Rows[i]["machineDescription"].ToString()] = downTimeFreq.Rows[i]["BreakDownCategoryFreq"];
                        downCode = downTimeFreq.Rows[i]["BreakDownCatagory"].ToString();
                        //  totalDown += Convert.ToInt32(downTimeFreq.Rows[i]["DownFreq"]);
                        Values.Rows[Values.Rows.Count - 1]["Total"] = downTimeFreq.Rows[i]["TotalBreakCategoryFreq"];

                        if (dicValue.ContainsKey(downTimeFreq.Rows[i]["machineDescription"].ToString()))
                            dicValue[downTimeFreq.Rows[i]["machineDescription"].ToString()] = Convert.ToInt32(downTimeFreq.Rows[i]["TotalMachineFreq"]);
                        else
                            dicValue.Add(downTimeFreq.Rows[i]["machineDescription"].ToString(), Convert.ToInt32(downTimeFreq.Rows[i]["TotalMachineFreq"]));
                    }
                    Values.Rows.Add(Total);
                    // answer = string.Empty;
                    for (int i = 0; i < dicValue.Count; i++)
        ...
        private void BindTimeWiseInfo()
        {
            try
            {
                ...
                DataTable dt = TMPTrakDataBase.MachineDownTimeMatrix(fromDate, toDate, machineSelection, plantId, downId, Exclude, "DTime", cellId, "s_GetSONA_BreakDownMatrix", "ByCategory");
                Session["DownTime"] = dt;
                DataView view = new DataView(dt);
                DataTable headerValues = view.ToTable(true, "machineDescription");
                DataTable Values = new DataTable();
                Values.Columns.Add("DownCode", typeof(string));
                for (int i = 0; i < headerValues.Rows.Count; i++)
                {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    Values.Rows[Values.Rows.Count - 1][dt.Rows[i]["machineDescription"].ToString()] = answer;
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    Values.Rows[Values.Rows.Count - 1][dt.Rows[i]["machineDescription"].ToString()] = dt.Rows[i]["DownFreq"];
                    downCode = dt.Rows[i]["DownCode"].ToString();
                }
                ...
                ///-----------------------------
                ///---------------------------------Down Freq-----------------------------------
                //DataTable dt2 = TMPTrakDataBase.MachineDownTimeMatrix(fromDate, toDate, machineSelection, plantId, downId, Exclude, "DFreqCategory");
        ...
        private void BindDownTimeFreqWise()
        {
            try
            {
                ...
                DataTable dt = TMPTrakDataBase.MachineDownTimeMatrix(fromDate, toDate, machineSelection, plantId, downId, Exclude, "DFreq", cellId, "s_GetSONA_BreakDownMatrix", "ByCategory");
        ...
        public static ChartData<SeriesTimeWise> MCsbyTopDownsChart(string plantID, string machineId, string startFromDate, string startToDate, string downID, bool exclude, int downReasons, string CellID)
        {
            ...
            try
            {
                ...
                if (HttpContext.Current.Session["DownTime"] == null)
                    dt = TMPTrakDataBase.MachineDownTimeMatrix(fromDate, toDate, machineId, plantId, downId, Exclude, "DTimeCategory", CellID, "s_GetSONA_BreakDownMatrix", "ByCategory");
                else
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    if (dt.Rows[i]["BreakDownCatagory"].ToString() != downCode)
                    {
                        row = Values.NewRow();
                        Values.Rows.Add(row);
                        Values.Rows[Values.Rows.Count - 1]["DownCode"] = dt.Rows[i]["BreakDownCatagory"];
                    }
                    //TimeSpan t = TimeSpan.FromSeconds(Convert.ToInt32(dt.Rows[i]["DownTime"]));
                    //string answer = string.Format("{0}",
                    //                t.TotalMinutes.ToString("00"));
                    Values.Rows[Values.Rows.Count - 1][dt.Rows[i]["machineDescription"].ToString()] = Convert.ToInt32(dt.Rows[i]["BreakDownCategoryTime"]);
                    downCode = dt.Rows[i]["BreakDownCatagory"].ToString();
                }
        ...
        public static ChartData<SeriesTimeWise> TopDownbyMCChart(string plantID, string machineId, string startFromDate, string startToDate, string downID, bool exclude, int downReasons, string CellID)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    Values.Rows[Values.Rows.Count - 1][dt.Rows[i]["machineDescription"].ToString()] = dt.Rows[i]["BreakDownCategoryTime"];
                    downCode = dt.Rows[i]["BreakDownCatagory"].ToString();
                }
        ...
        public static ChartData<SeriesTimeWise> MCsbyTopDownFreqChart(string plantID, string machineId, string startFromDate, string startToDate, string downID, bool exclude, int downReasons, string CellID)
        {
            ...
            try
            {
                ...
                if (HttpContext.Current.Session["DownTimeFreq"] == null)
                    dt = TMPTrakDataBase.MachineDownTimeMatrix(fromDate, toDate, machineId, plantId, downId, Exclude, "DFreqCategory", CellID, "s_GetSONA_BreakDownMatrix", "ByCategory");
                else
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    Values.Rows[Values.Rows.Count - 1][dt.Rows[i]["machineDescription"].ToString()] = Convert.ToInt32(dt.Rows[i]["BreakDownCategoryFreq"]);
                    downCode = dt.Rows[i]["BreakDownCatagory"].ToString();
                }
        ...
        public static ChartData<SeriesTimeWise> TopDownFreqbyMCsChart(string plantID, string machineId, string startFromDate, string startToDate, string downID, bool exclude, int downReasons, string CellID)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    Values.Rows[Values.Rows.Count - 1][dt.Rows[i]["machineDescription"].ToString()] = dt.Rows[i]["BreakDownCategoryFreq"];
                    downCode = dt.Rows[i]["BreakDownCatagory"].ToString();
                }
        ...
        public static Chart<DrildownData> TopDownTimeParetoChart(string plantID, string machineId, string startFromDate, string startToDate, string downID, bool exclude, int downReasons, string CellID)
        {
            ...
            try
            {
                ...
                if (HttpContext.Current.Session["DownParetoData"] == null)
                    HttpContext.Current.Session["DownParetoData"] = dt = TMPTrakDataBase.MachineDownTimeMatrix(fromDate, toDate, machineId, plantId, downId, Exclude, "DLossCatagory", CellID, "s_GetSONA_BreakDownMatrix", "ByCategory");
                else
        ...
        //        DataTable dt = null;
        //        if (HttpContext.Current.Session["DownParetoData"] == null)
        //            HttpContext.Current.Session["DownParetoData"] = dt = TMPTrakDataBase.MachineDownTimeMatrix(fromDate, toDate, machineId, plantId, downId, Exclude, "DLossCatagory");
        //        else
        //            dt = HttpContext.Current.Session["DownParetoData"] as DataTable;
        ...
        public static Chart<DrildownData> TopDownTimeParetoChartColAndPie(string plantID, string machineId, string startFromDate, string startToDate, string downID, bool exclude, int downReasons, string CellID)
        {
            ...
            try
            {
                ...
                if (HttpContext.Current.Session["DownParetoData"] == null)
                    ...
                    dt = HttpContext.Current.Session["DownParetoData"] as DataTable;
                DataView dv = dt.DefaultView;
                dv.Sort = "BreakDownCategoryTime desc";
                DataTable sortedDT = dv.ToTable();

                chart.series = new List<DrildownData>();
                List<string> cat = sortedDT.AsEnumerable().Select(r => r.Field<string>("BreakDownCatagory")).ToList();
                ...
                for (int i = 0; i < sortedDT.Rows.Count; i++)
                {
                    ...
                    {
                        flag = false;
                        List<double> declistVal = sortedDT.AsEnumerable().Select(r => r.Field<double>("BreakDownCategoryTime")).Skip(downReasons).ToList();
                        for (int j = 0; j < declistVal.Count; j++)
                        {
                            decVal += Convert.ToDecimal(declistVal[j]);
                        }
                    ...
                    {
                        //decVal += Convert.ToDecimal(sortedDT.Rows[i]["DownTime"].ToString());
                        decimal myNumber = Convert.ToDecimal(sortedDT.Rows[i]["BreakDownCategoryTime"].ToString());
                        obj.y = decimal.Round(myNumber, 2);
                        obj.name = sortedDT.Rows[i]["BreakDownCatagory"].ToString();
                    }
                    chart.series.Add(obj);
                ...
                foreach (var machine in rows)
                {
                    chart.drilldown[0].data.Add(new DrildownData
                    {
                        name = machine[0].ToString(),
                        y = Convert.ToDecimal(machine[1].ToString()),
                    });
        ...
        public static Chart<Series> CatagoryByDownTimeParetoChart(string plantID, string machineId, string startFromDate, string startToDate, string downID, bool exclude, int downReasons, string CellID)
        {
            ...
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    sortedDT = dt.AsEnumerable()
                                     //.OrderBy(r => r.Field<double>("DownTime"))
                                     .GroupBy(r => r.Field<string>("BreakDownCatagory").ToUpper())
                                     .OrderByDescending(g => g.Max(r => r.Field<double>("BreakDownTime")))
                                     .Select(g =>
                                     {
                                         var row = dt.NewRow();
                                         row["BreakDownCatagory"] = g.Key.ToUpper();
                                         row["BreakDownTime"] = g.Sum(r => r.Field<double>("BreakDownTime"));
                                         return row;
                                     }).CopyToDataTable();
                }
                //List<string> cat = sortedDT.AsEnumerable().Select(r => r.Field<string>("Catagory")).ToList();
                ...
                {
                    Data obj = new Data();
                    decimal myNumber = Convert.ToDecimal(sortedDT.Rows[i]["BreakDownTime"].ToString());
                    obj.y = decimal.Round(myNumber, 2);
                    obj.name = sortedDT.Rows[i]["BreakDownCatagory"].ToString();
                    obj.drilldown = sortedDT.Rows[i]["BreakDownCatagory"].ToString();
                    chart.series[0].data.Add(obj);
                }
                ...
                for (int i = 0; i < sortedDT.Rows.Count; i++)
                {
                    chart.drilldown.Add(new DrildownSeries
                    {
                        colorByPoint = true,
                        type = "column",
                        name = sortedDT.Rows[i]["BreakDownCatagory"].ToString(),
                        id = sortedDT.Rows[i]["BreakDownCatagory"].ToString(),
                        data = new List<DrildownData>(),
                    });
                    var results = dt.Select("BreakDownCatagory = '" + sortedDT.Rows[i]["BreakDownCatagory"].ToString() + "'").OrderByDescending(dr => dr[2]);
                    if (results != null)
                    {
                        foreach (DataRow drCode in results)
                        {
                            chart.drilldown[i].data.Add(new DrildownData
                            {
                                name = drCode["BreakDownCode"].ToString(),
                                y = Convert.ToDecimal(drCode["BreakDownTime"].ToString()),
                            });
