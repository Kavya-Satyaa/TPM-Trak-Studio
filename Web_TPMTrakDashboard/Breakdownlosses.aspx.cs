...
{
    public partial class Breakdownlosses : System.Web.UI.Page
    {
        ...
        private async Task GetAsync()
        {
            try
            {
                ...
                else
                {
                    ...
                    var vardownTime = TMPTrakDataBase.GetBreakDownTimeAndFreqData(fromDate, toDate, machineSelection, plantId, downId, Exclude, "DTime", cellId, "ByPhenomenon");
                    var vardownTimeFreq = TMPTrakDataBase.GetBreakDownTimeAndFreqData(fromDate, toDate, machineSelection, plantId, downId, Exclude, "DFreq", cellId, "ByPhenomenon");
                    ...
                    {
                        if (downTimeWise.Rows[i]["BreakDownCode"].ToString() != downCode)
                        {
                            row = Values.NewRow();
                            Values.Rows.Add(row);
                            Values.Rows[Values.Rows.Count - 1]["DownCode"] = downTimeWise.Rows[i]["BreakDownCode"];
                        }
                        if (Convert.ToInt32(downTimeWise.Rows[i]["BreakDownTime"]) > 0)
                        {
                            t = TimeSpan.FromSeconds(Convert.ToInt32(downTimeWise.Rows[i]["BreakDownTime"]));
                            int seconds = Convert.ToInt32(downTimeWise.Rows[i]["BreakDownTime"]);
                        ...
                        downCode = downTimeWise.Rows[i]["BreakDownCode"].ToString();
                    ...
                    for (int i = 0; i < downTimeWise.Rows.Count; i++)
                    {
                        ...
                        Values.Rows[Values.Rows.Count - 1][downTimeWise.Rows[i]["machineDescription"].ToString()] = downTimeWise.Rows[i]["BreakDownFreq"];
                        downCode = downTimeWise.Rows[i]["BreakDownCode"].ToString();
                        //totalDown += Convert.ToInt32(downTimeWise.Rows[i]["DownFreq"]);
                        Values.Rows[Values.Rows.Count - 1]["Total"] = downTimeWise.Rows[i]["TotalBreakDownFreq"];
                    ...
                    {
                        if (downTimeFreq.Rows[i]["BreakDownCode"].ToString() != downCode)
                        {
                            row = Values.NewRow();
                            Values.Rows.Add(row);
                            Values.Rows[Values.Rows.Count - 1]["DownCode"] = downTimeFreq.Rows[i]["BreakDownCode"];
                            //totalDown = 0;
                        }
                        Values.Rows[Values.Rows.Count - 1][downTimeFreq.Rows[i]["machineDescription"].ToString()] = downTimeFreq.Rows[i]["BreakDownFreq"];
                        downCode = downTimeFreq.Rows[i]["BreakDownCode"].ToString();
                        //  totalDown += Convert.ToInt32(downTimeFreq.Rows[i]["DownFreq"]);
                        Values.Rows[Values.Rows.Count - 1]["Total"] = downTimeFreq.Rows[i]["TotalBreakDownFreq"];
        ...
        private void BindTimeWiseInfo()
        {
            try
            {
                ...
                DataTable dt = TMPTrakDataBase.MachineDownTimeMatrix(fromDate, toDate, machineSelection, plantId, downId, Exclude, "DTime", cellId, "s_GetSONA_BreakDownMatrix", "ByPhenomenon");
                Session["DownTime"] = dt;
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    Values.Rows[Values.Rows.Count - 1][dt.Rows[i]["MachineID"].ToString()] = answer;
                ...
                ///-----------------------------
                ///---------------------------------Down Freq-----------------------------------
                //DataTable dt2 = TMPTrakDataBase.MachineDownTimeMatrix(fromDate, toDate, machineSelection, plantId, downId, Exclude, "DFreq", "ByPhenomenon");
        ...
        private void BindDownTimeFreqWise()
        {
            try
            {
                ...
                DataTable dt = TMPTrakDataBase.MachineDownTimeMatrix(fromDate, toDate, machineSelection, plantId, downId, Exclude, "DFreq", cellId, "s_GetSONA_BreakDownMatrix", "ByPhenomenon");
        ...
        public static ChartData<SeriesTimeWise> MCsbyTopDownsChart(string plantID, string machineId, string startFromDate, string startToDate, string downID, bool exclude, int downReasons, string CellID)
        {
            ...
            try
            {
                ...
                if (HttpContext.Current.Session["DownTime"] == null)
                    dt = TMPTrakDataBase.MachineDownTimeMatrix(fromDate, toDate, machineId, plantId, downId, Exclude, "DTime", CellID, "s_GetSONA_BreakDownMatrix", "ByPhenomenon");
                else
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    if (dt.Rows[i]["BreakDownCode"].ToString() != downCode)
                    {
                        row = Values.NewRow();
                        Values.Rows.Add(row);
                        Values.Rows[Values.Rows.Count - 1]["DownCode"] = dt.Rows[i]["BreakDownCode"];
                    }
                    //TimeSpan t = TimeSpan.FromSeconds(Convert.ToInt32(dt.Rows[i]["DownTime"]));
                    //string answer = string.Format("{0}",
                    //                t.TotalMinutes.ToString("00"));
                    Values.Rows[Values.Rows.Count - 1][dt.Rows[i]["machineDescription"].ToString()] = Convert.ToInt32(dt.Rows[i]["BreakDownTime"]);
                    downCode = dt.Rows[i]["BreakDownCode"].ToString();
                }
        ...
        public static ChartData<SeriesTimeWise> TopDownbyMCChart(string plantID, string machineId, string startFromDate, string startToDate, string downID, bool exclude, int downReasons, string CellID)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    Values.Rows[Values.Rows.Count - 1][dt.Rows[i]["machineDescription"].ToString()] = dt.Rows[i]["BreakDownTime"];
                    downCode = dt.Rows[i]["BreakDownCode"].ToString();
                }
        ...
        public static ChartData<SeriesTimeWise> MCsbyTopDownFreqChart(string plantID, string machineId, string startFromDate, string startToDate, string downID, bool exclude, int downReasons, string CellID)
        {
            ...
            try
            {
                ...
                if (HttpContext.Current.Session["DownTimeFreq"] == null)
                    dt = TMPTrakDataBase.MachineDownTimeMatrix(fromDate, toDate, machineId, plantId, downId, Exclude, "DFreq", CellID, "s_GetSONA_BreakDownMatrix", "ByPhenomenon");
                else
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    Values.Rows[Values.Rows.Count - 1][dt.Rows[i]["machineDescription"].ToString()] = Convert.ToInt32(dt.Rows[i]["BreakDownFreq"]);
                    downCode = dt.Rows[i]["BreakDownCode"].ToString();
                }
        ...
        public static ChartData<SeriesTimeWise> TopDownFreqbyMCsChart(string plantID, string machineId, string startFromDate, string startToDate, string downID, bool exclude, int downReasons, string CellID)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    Values.Rows[Values.Rows.Count - 1][dt.Rows[i]["machineDescription"].ToString()] = dt.Rows[i]["BreakDownFreq"];
                    downCode = dt.Rows[i]["BreakDownCode"].ToString();
                }
        ...
        public static Chart<DrildownData> TopDownTimeParetoChart(string plantID, string machineId, string startFromDate, string startToDate, string downID, bool exclude, int downReasons, string CellID)
        {
            ...
            try
            {
                ...
                if (HttpContext.Current.Session["DownParetoData"] == null)
                    HttpContext.Current.Session["DownParetoData"] = dt = TMPTrakDataBase.MachineDownTimeMatrix(fromDate, toDate, machineId, plantId, downId, Exclude, "DLoss_By_Catagory", CellID, "s_GetSONA_BreakDownMatrix", "ByPhenomenon");
                else
        ...
        public static Chart<DrildownData> TopDownTimeParetoChartColAndPie(string plantID, string machineId, string startFromDate, string startToDate, string downID, bool exclude, int downReasons, string CellID)
        {
            ...
            try
            {
                ...
                if (HttpContext.Current.Session["DownParetoData"] == null)
                    ...
                    dt = HttpContext.Current.Session["DownParetoData"] as DataTable;
                DataView dv = dt.DefaultView;
                dv.Sort = "BreakDownTime desc";
                DataTable sortedDT = dv.ToTable();

                chart.series = new List<DrildownData>();
                List<string> cat = sortedDT.AsEnumerable().Select(r => r.Field<string>("BreakDownCode")).ToList();
                ...
                for (int i = 0; i < sortedDT.Rows.Count; i++)
                {
                    ...
                    {
                        flag = false;
                        List<double> declistVal = sortedDT.AsEnumerable().Select(r => r.Field<double>("BreakDownTime")).Skip(downReasons).ToList();
                        for (int j = 0; j < declistVal.Count; j++)
                        {
                            decVal += Convert.ToDecimal(declistVal[j]);
                        }
                    ...
                    if (flag)
                    {
                        ...
                        obj.y = decimal.Round(myNumber, 2);
                        obj.name = sortedDT.Rows[i]["BreakDownCode"].ToString();
                    }
                    chart.series.Add(obj);
