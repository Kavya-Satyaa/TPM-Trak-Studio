<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="FiveSChecksheetTransDenso.aspx.cs" Inherits="Web_TPMTrakDashboard.Denso.FiveSChecksheetTransDenso" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
                    ...
                    <ItemTemplate>
                        <tr>
                            ...
                            <td style='display: <%# Eval("RowDisplay") %>; white-space: nowrap' rowspan='<%# Eval("RowSpan") %>'>
                                ...
                                <asp:Label runat="server" ClientIDMode="Static" Visible='<%# Eval("ContentVisibility") %>' Text='<%# Eval("CycleTimeDisplay") %>'></asp:Label>
                            </td>
                            <td>
                                    ...
                                    <ItemTemplate>
                                        <td class="td-actual-value">
                                            ...
                                            <div runat="server" visible='<%# Eval("ContentVisibility") %>'>
                                                ...
                                                <div runat="server" style='<%# "background-color:" + Eval("CheckpointTypeBackColor") + "; color:" + Eval("CheckpointTypeForeColor") %>' visible='<%# Eval("CheckpointType").ToString() == "OK-NG" || Eval("CheckpointType").ToString() == "Yes-No" %>'>
    ...
    <script>
        ...
        function freezeColumnLeft(tblId, freezColNo) {            debugger;            let tblName = tblId;            let tblRow = $('[id$=' + tblName + '] > tbody > tr');            if (tblRow.length > 0) {                let rowth = tblRow[0].children;                var width = 0;                let columnCount = freezColNo;                for (var i = 0; i < columnCount; i++) {
                    let zindex = 800;
                    $("#" + tblName + " > tbody > tr:first-child > td:nth-child(" + (i + 1) + ")").css({
                        "position": "sticky",
                        "left": width + "px",
                        "z-index": "999"
                    });
                ...
                }                //$("#" + tblId + " > tbody > tr:first-child td:lt(" + (columnCount+1) + ")").css({ "z-index": "999" });            }        }
