...
{
    public partial class DownTimePieChart : System.Web.UI.Page
    {
        public string chartType = string.Empty;
        public string timeTex = string.Empty;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                if (Request.QueryString["machineId"] != "")
                {
                    ...
                    Session["timeTex"] = timeTex = Request.QueryString["timeTex"].ToString();
                }
            }
        ...
        {
            string timeText = HttpContext.Current.Session["timeTex"] == null ? "hh" : HttpContext.Current.Session["timeTex"].ToString();
            string date = string.Empty;
            bool legend = type == "pie" ? true : false;
            var chart = new Chart<Series>
            {
                Title = "TITLE",
                Subtitle = "SubTitle",
                XAxisTitle = HttpContext.GetLocalResourceObject("~/DownTimePieChart.aspx", "DownTimeByDownCategory").ToString(),//_DDCategory,
                YAxisTitle = HttpContext.GetLocalResourceObject("~/DownTimePieChart.aspx", "Timein").ToString() + " (" + timeText + ")",
                legend = legend,
            ...
            try
            {
                ...
                colorByPoint = (type == "pie" || type == "column") ? true : false;
                ...
                if (strYear != "" && strMonth != "" && strDay != "")
                {
                    ...
                    foreach (DataRow item in dtMonth.Rows)
                    {
                        ...
                        chart.series[0].data.Add(new Data
                        {
                            ...
                            y = Convert.ToDecimal(item["TotalDown"].ToString()),
                            drilldown = item["Catagory"].ToString().Trim().ToUpper() + "-" + Convert.ToDateTime(item["PDate"]).ToString("MM") + "-" + strDay,
                    ...
                    foreach (var catagory in categoryData)
                    {
                        chart.drilldown.Add(new DrildownSeries
                        {
                            ...
                            type = type,
                            colorByPoint = colorByPoint,
                            id = catagory.Trim().ToUpper() + "-" + strMonth + "-" + strDay,
                        ...
                        foreach (DataRow drshift in results)
                        {
                            chart.drilldown[i].data.Add(new DrildownData
                            {
                                ...
                                y = Convert.ToDecimal(drshift["TotalDown"].ToString()),
                                afterTitel = drshift["Catagory"].ToString() + " (" + strYear + "-" + drshift["Month"].ToString() + "-" + drshift["Day"].ToString() + "-" + drshift["Shift"].ToString() + ")",
                                beforeTitle = "Down Time By Down Category" + " : " + drshift["Catagory"].ToString() + " (" + strYear + "-" + drshift["Month"].ToString() + "-" + drshift["Day"].ToString() + ")"
                ...
                else if (strYear != "" && strMonth != "")
                {
                    ...
                    foreach (DataRow item in dtMonth.Rows)
                    {
                        ...
                        chart.series[0].data.Add(new Data
                        {
                            name = item["Catagory"].ToString(),
                            y = Convert.ToDecimal(item["TotalDown"].ToString()),
                            drilldown = item["Catagory"].ToString().Trim().ToUpper(),
                    ...
                    foreach (var catagory in categoryData)
                    {
                        ...
                        foreach (DataRow dr in results)
                        {
                            chart.drilldown[i].data.Add(new DrildownData
                            {
                                ...
                                y = Convert.ToDecimal(dr["TotalDown"].ToString()),
                                drilldown = dr["Catagory"].ToString().Trim().ToUpper() + "/" + strMonth + "/" + Convert.ToDateTime(dr["PDate"]).ToString("dd"),
                                afterTitel = dr["Catagory"].ToString() + " (" + strYear + "-" + strMonth + "-" + Convert.ToDateTime(dr["PDate"]).ToString("dd") + ")",
                                beforeTitle = "Down Time By Down Category" + " : " + dr["Catagory"].ToString() + " (" + strYear + "-" + strMonth + ")"
                            });
                    ...
                    foreach (var catagory in categoryData)
                    {
                        foreach (var day in monthData)
                        {
                            chart.drilldown.Add(new DrildownSeries
                            {
                                ...
                                type = type,
                                colorByPoint = colorByPoint,
                                id = catagory.Trim().ToUpper() + "/" + strMonth + "/" + day,
                                data = new List<DrildownData>(),
                            });
                        ...
                        if (results != null)
                        {
                            foreach (DataRow drmonth in results)
                            {
                                chart.drilldown[i].data.Add(new DrildownData
                                {
                                    ...
                                    y = Convert.ToDecimal(drmonth["TotalDown"].ToString()),
                                    drilldown = drmonth["Catagory"].ToString().ToUpper() + "-" + Convert.ToDateTime(drmonth["Startdate"]).ToString("MM"),
                                    afterTitel = drmonth["Catagory"].ToString() + " (" + strYear + "-" + Convert.ToDateTime(drmonth["Startdate"]).ToString("MM") + ")",
                                    beforeTitle = "Down Time By Down Category" + " : " + drmonth["Catagory"].ToString() + " (" + strYear + ")"
                                });
                            }
                            i++;
                        }
                    ...
                    foreach (var catagory in categoryData)
                    {
                        foreach (var month in monthName)
                        {
                            chart.drilldown.Add(new DrildownSeries
                            {
                                ...
                                type = type,
                                colorByPoint = colorByPoint,
                                id = catagory.Trim().ToUpper() + "-" + month,
                                data = new List<DrildownData>(),
                            });
                            ...
                            if (results != null)
                            {
                                foreach (DataRow dr in results)
                                {
                                    string ss = dr["Catagory"].ToString() + "#@$&*" + month + "#@$&*" + Convert.ToDateTime(dr["PDate"]).ToString("dd");
                                    chart.drilldown[i].data.Add(new DrildownData
                                    {
                                        name = "Day" + " " + Convert.ToDateTime(dr["PDate"]).ToString("dd"),
                                        y = Convert.ToDecimal(dr["TotalDown"].ToString()),
                                        ...
                                        beforeTitle = "Down Time By Down Category" + " : " + dr["Catagory"].ToString() + " (" + strYear + "-" + month + ")"
                                    });
                                }
                                i++;
                            }
        ...
        public static Chart<Series> GetDownCodeChart(string param, string type, string category)
        {
            ...
            var chart = new Chart<Series>
            {
                Title = "TITLE",
                Subtitle = "SubTitle",
                XAxisTitle = HttpContext.GetLocalResourceObject("~/DownTimePieChart.aspx", "DownTimeByDownCode").ToString(),//DDCode,
                YAxisTitle = HttpContext.GetLocalResourceObject("~/DownTimePieChart.aspx", "Timein").ToString() + " (" + timeText + ")",
                legend = legend,
                YAxisTooltipValueSuffix = "YAxisTooltipValueSuffix"
            };
                ...
                if (strYear != "" && strMonth != "" && strDay != "")
                {
                    ...
                    foreach (DataRow item in catMonth)
                    {
                        ...
                        chart.series[0].data.Add(new Data
                        {
                            ...
                            y = Convert.ToDecimal(item["Down"].ToString()),
                    ...
                    foreach (var code in codeData)
                    {
                        ...
                        foreach (DataRow drshift in results)
                        {
                            ...
                            {
                                name = drshift["Shift"].ToString(),
                                y = Convert.ToDecimal(drshift["Down"].ToString()),
                                afterTitel = drshift["Code"].ToString() + " (" + strYear + "-" + drshift["Month"].ToString() + "-" + drshift["Day"].ToString() + "-" + drshift["Shift"].ToString() + ")",
                                beforeTitle = "Down Time By Down Code" + " : " + drshift["Code"].ToString() + " (" + strYear + "-" + drshift["Month"].ToString() + "-" + drshift["Day"].ToString() + ")"
                ...
                else if (strYear != "" && strMonth != "")
                {
                    ...
                    foreach (var code in codeData)
                    {
                        ...
                        foreach (DataRow dr in results)
                        {
                            ...
                            {
                                name = "Day" + " " + Convert.ToDateTime(dr["PDate"]).ToString("dd"),
                                y = Convert.ToDecimal(dr["Down"].ToString()),
                                ...
                                beforeTitle = "Down Time By Down Code" + " : " + dr["Code"].ToString() + " (" + strYear + "-" + strMonth + ")"
                            });
                            monthData.Add(Convert.ToDateTime(dr["PDate"]).ToString("dd"));
                        }
                        i++;
                    ...
                    foreach (var code in codeData)
                    {
                        ...
                        if (results != null)
                        {
                            foreach (DataRow drmonth in results)
                            {
                                ...
                                {
                                    name = drmonth["NameOftheMonth"].ToString(),
                                    y = Convert.ToDecimal(drmonth["Down"].ToString()),
                                    ...
                                    beforeTitle = "Down Time By Down Code" + " : " + drmonth["Code"].ToString() + " (" + strYear + ")"
                                });
                            }
                            i++;
                        }
                            ...
                            if (results != null)
                            {
                                foreach (DataRow dr in results)
                                {
                                    chart.drilldown[i].data.Add(new DrildownData
                                    {
                                        ...
                                        beforeTitle = "Down Time By Down Code" + " : " + dr["Code"].ToString() + " (" + strYear + "-" + month + ")"
                                    });
                                }
                                i++;
                            }
        ...
        public static Chart<Series> GetShiftChartData(string strMonth, string strDay, string param, string type, string catagory, string code)
        {
            ...
            var chart = new Chart<Series>
            {
                Title = "TITLE",
                Subtitle = "SubTitle",
                XAxisTitle = (param == "DownCategory" ? HttpContext.GetLocalResourceObject("~/DownTimePieChart.aspx", "DownTimeByDownCategory").ToString() : HttpContext.GetLocalResourceObject("~/DownTimePieChart.aspx", "DownTimeByDownCode").ToString()) + " : " + xasis + " (" + strYear + "-" + strMonth + "-" + strDay + ")",
                YAxisTitle = HttpContext.GetLocalResourceObject("~/DownTimePieChart.aspx", "Timein").ToString() + " (" + timeText + ")",
                YAxisTooltipValueSuffix = "YAxisTooltipValueSuffix"
            };
            DataTable dtYear = new DataTable();
        ...
        public static void SaveDownIDLabel(string DownID)
        {
            try
            {
                HttpContext.Current.Session["DownID"] = DownID;
            }
            catch (Exception ex)
            {
                Logger.WriteErrorLog("SaveDownIDLabel: " + ex.Message);
            }
        }
        ...
        public static void SaveMonth(string Month)
        {
            try
            {
                HttpContext.Current.Session["MonthClicked"] = Month;
            }
            catch (Exception ex)
            {
                Logger.WriteErrorLog("SaveDownIDLabel: " + ex.Message);
