...
{
    public partial class DowntimePage : System.Web.UI.Page
    {
        ...
        public static Chart<Series> GetDownCategoryChart(string param, string type)
        {
            ...
            string strShift = HttpContext.Current.Session["ShiftId"].ToString(), plantId = HttpContext.Current.Session["plantId"].ToString(),
             strMonth = HttpContext.Current.Session["Month"].ToString(), componentId = HttpContext.Current.Session["componentId"].ToString(),
             employeeId = HttpContext.Current.Session["employeeId"].ToString(), strYear = HttpContext.Current.Session["Year"].ToString(),
             strDay = HttpContext.Current.Session["Day"].ToString(), machineId = HttpContext.Current.Session["MachineId"].ToString(), CellID = HttpContext.Current.Session["CellID"].ToString();
            ...
            try
            {
                ...
                if (strYear != "" && strMonth != "" && strDay != "")
                {
                    ...
                    dtMonth = BindCockpitView.BindRejRewDowntimeInfo(date, strShift, plantId, machineId, "currentDAY", param, componentId, CellID, employeeId, "");

                    foreach (DataRow item in dtMonth.Rows)
                    {
                        categoryData.Add(item["Catagory"].ToString());
                        chart.series[0].data.Add(new Data { name = item["Catagory"].ToString(), y = Convert.ToDecimal(item["TotalDown"].ToString()), drilldown = item["Catagory"].ToString() + "-" + Convert.ToDateTime(item["PDate"]).ToString("MM") + "-" + strDay });
                    }
                    //-------------------------------Shift Wise Info-----------------------------------
                    chart.drilldown = new List<DrildownSeries>();
                    int i = 0;
                    dtShift = BindCockpitView.BindRejRewDowntimeInfo(date, strShift, plantId, machineId, "SHIFT", param, componentId, CellID, employeeId, "");
                    foreach (var catagory in categoryData)
                    {
                        ...
                        {
                            chart.drilldown[i].data.Add(new DrildownData { name = drshift["Shift"].ToString(), y = Convert.ToDecimal(drshift["TotalDown"].ToString()) }); /// drilldown = drshift["Catagory"].ToString() + "/" + drshift["Month"].ToString() + "/" + drshift["Day"].ToString() + "/" + drshift["Shift"].ToString()
                        }
                        i++;
                    }
                    #endregion
                ...
                else if (strYear != "" && strMonth != "")
                {
                    ...
                    dtMonth = BindCockpitView.BindRejRewDowntimeInfo(date, strShift, plantId, machineId, "currentMONTH", param, componentId, CellID, employeeId, "");

                    foreach (DataRow item in dtMonth.Rows)
                    {
                        categoryData.Add(item["Catagory"].ToString());
                        chart.series[0].data.Add(new Data { name = item["Catagory"].ToString(), y = Convert.ToDecimal(item["TotalDown"].ToString()), drilldown = item["Catagory"].ToString() });
                    }

                    //---------------------------Day Wise Start---------------------------------
                    chart.drilldown = new List<DrildownSeries>();
                    int i = 0;
                    dtDay = BindCockpitView.BindRejRewDowntimeInfo(date, strShift, plantId, machineId, "DAY", param, componentId, CellID, employeeId, "");
                    foreach (var catagory in categoryData)
                    {
                        chart.drilldown.Add(new DrildownSeries
                        {
                        ...
                        {
                            chart.drilldown[i].data.Add(new DrildownData { name = "Day" + " " + Convert.ToDateTime(dr["PDate"]).ToString("dd"), y = Convert.ToDecimal(dr["TotalDown"].ToString()), drilldown = dr["Catagory"].ToString() + "/" + strMonth + "/" + Convert.ToDateTime(dr["PDate"]).ToString("dd") });
                ...
                else if (strYear != "")
                {
                    ...
                    date = strYear + "-" + "01" + "-" + "01";
                    dtYear = BindCockpitView.BindRejRewDowntimeInfo(date, strShift, plantId, machineId, "YEAR", param, componentId, CellID, employeeId, "");
                    ...
                    dtMonth = BindCockpitView.BindRejRewDowntimeInfo(date, strShift, plantId, machineId, "MONTH", param, componentId, CellID, employeeId, "");
                    chart.drilldown = new List<DrildownSeries>();
                    int i = 0;
                    foreach (var catagory in categoryData)
                    {
                        ...
                        if (results != null)
                        {
                            ...
                            {
                                chart.drilldown[i].data.Add(new DrildownData { name = drmonth["NameOftheMonth"].ToString(), y = Convert.ToDecimal(drmonth["TotalDown"].ToString()), drilldown = drmonth["Catagory"].ToString() + "-" + Convert.ToDateTime(drmonth["Startdate"]).ToString("MM") });
                            }
                            i++;
                        }
                    }
                    ...
                    foreach (var catagory in categoryData)
                    {
                        foreach (var month in monthName)
                        {
                            ...
                            if (results != null)
                            {
                                foreach (DataRow dr in results)
                                {
                                    chart.drilldown[i].data.Add(new DrildownData { name = "Day" + " " + Convert.ToDateTime(dr["PDate"]).ToString("dd"), y = Convert.ToDecimal(dr["TotalDown"].ToString()), drilldown = dr["Catagory"].ToString() + "#@$&*" + month + "#@$&*" + Convert.ToDateTime(dr["PDate"]).ToString("dd") });
                                }
                                i++;
                            }
                        }
        ...
        [WebMethod]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public static Chart<Series> GetDownCodeChart(string param, string type, string category)
        {
            string date = string.Empty;
            var chart = new Chart<Series>
            {
            ...
            try
            {
                ...
                if (strYear != "" && strMonth != "" && strDay != "")
                {
                    ...
                    foreach (DataRow item in catMonth)
                    {
                        ...
                        chart.series[0].data.Add(new Data { name = item["Code"].ToString(), y = Convert.ToDecimal(item["Down"].ToString()), drilldown = Convert.ToDateTime(item["PDate"]).ToString("MM") + "/" + strDay + "/" + item["Code"].ToString() });
                    }
                    //-------------------------------Shift Wise Info-----------------------------------
                    chart.drilldown = new List<DrildownSeries>();
                    int i = 0;
                    ...
                    foreach (var code in codeData)
                    {
                        ...
                        foreach (DataRow drshift in results)
                        {
                            chart.drilldown[i].data.Add(new DrildownData { name = drshift["Shift"].ToString(), y = Convert.ToDecimal(drshift["Down"].ToString()) });//, drilldown = drshift["Code"].ToString() + "/" + drshift["Month"].ToString() + "/" + drshift["Day"].ToString() + "/" + drshift["Shift"].ToString()
                        }
                        i++;
                    }
                    #endregion
                ...
                else if (strYear != "" && strMonth != "")
                {
                    ...
                    {
                        codeData.Add(item["Code"].ToString());
                        chart.series[0].data.Add(new Data { name = item["Code"].ToString(), y = Convert.ToDecimal(item["Down"].ToString()), drilldown = item["Code"].ToString() });
                    }
                    ...
                    foreach (var code in codeData)
                    {
                        ...
                        foreach (DataRow dr in results)
                        {
                            chart.drilldown[i].data.Add(new DrildownData { name = "Day" + " " + Convert.ToDateTime(dr["PDate"]).ToString("dd"), y = Convert.ToDecimal(dr["Down"].ToString()), drilldown = dr["Code"].ToString() + "/" + strMonth + "/" + Convert.ToDateTime(dr["PDate"]).ToString("dd") });
                            monthData.Add(Convert.ToDateTime(dr["PDate"]).ToString("dd"));
                        }
                        i++;
                    }
                ...
                else if (strYear != "")
                {
                    ...
                    foreach (var code in codeData)
                    {
                        ...
                        if (results != null)
                        {
                            foreach (DataRow drmonth in results)
                            {
                                chart.drilldown[i].data.Add(new DrildownData { name = drmonth["NameOftheMonth"].ToString(), y = Convert.ToDecimal(drmonth["Down"].ToString()), drilldown = drmonth["Code"].ToString() + "-" + Convert.ToDateTime(drmonth["Startdate"]).ToString("MM") });
                            }
                            i++;
                        }
                    }
                    ...
                    foreach (var code in codeData)
                    {
                        foreach (var month in monthName)
                        {
                            ...
                            if (results != null)
                            {
                                foreach (DataRow dr in results)
                                {
                                    chart.drilldown[i].data.Add(new DrildownData { name = "Day" + " " + Convert.ToDateTime(dr["PDate"]).ToString("dd"), y = Convert.ToDecimal(dr["Down"].ToString()), drilldown = dr["Code"].ToString() + "#@$&*" + month + "#@$&*" + Convert.ToDateTime(dr["PDate"]).ToString("dd") });
                                }
                                i++;
                            }
                        }
        ...
        {
            string strShift = HttpContext.Current.Session["ShiftId"].ToString(), plantId = HttpContext.Current.Session["plantId"].ToString(),
            employeeId = HttpContext.Current.Session["employeeId"].ToString(), componentId = HttpContext.Current.Session["componentId"].ToString(),
            machineId = HttpContext.Current.Session["MachineId"].ToString(), strYear = HttpContext.Current.Session["Year"].ToString(),  CellID = HttpContext.Current.Session["CellID"].ToString();
            ...
            {
                date = strYear + "-" + strMonth + "-" + strDay;
                dtYear = BindCockpitView.BindRejRewDowntimeInfo(date, strShift, plantId, machineId, "CurrentSHIFT", param, componentId, CellID, employeeId, "");
                if (type == "")
                {
                    type = HttpContext.Current.Session["chartType"].ToString();
                }
                ...
                {
                    if (catagory != "")
                        chart.series[0].data.Add(new Data { name = item["Shift"].ToString(), y = Convert.ToDecimal(item["TotalDown"].ToString()) }); //, drilldown = item["MachineID"].ToString() + "/" + item["Month"].ToString() + "/" + item["Day"].ToString() + "/" + item["Shift"].ToString()
                    else
                        chart.series[0].data.Add(new Data { name = item["Shift"].ToString(), y = Convert.ToDecimal(item["Down"].ToString()) }); //, drilldown = item["MachineID"].ToString() + "/" + item["Month"].ToString() + "/" + item["Day"].ToString() + "/" + item["Shift"].ToString()
                }
            }
            catch (Exception ex)
            {
