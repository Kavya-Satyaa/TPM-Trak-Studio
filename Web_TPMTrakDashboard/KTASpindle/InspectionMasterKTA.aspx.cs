...
namespace Web_TPMTrakDashboard.KTASpindle
{
    public partial class InspectionMaster : System.Web.UI.Page
    {
        ...
        static string appPath = HttpContext.Current.Server.MapPath("~/KTASpindle/ReportTemplates");
        ...
        private void BindPartFamily()
        {
            try
            {
                List<string> lstPartFamily = TMPTrakDataBase.GetAllPartFamily();
                ddlPartFamily.DataSource = lstPartFamily;
                ddlPartFamily.DataBind();

                ddlCopyPartFamily.DataSource = lstPartFamily;
                ddlCopyPartFamily.DataBind();
        ...
        private void BindComponentId()
        {
            try
            {
                List<string> lstComponentID = TMPTrakDataBase.GetAllComponentByPartFamily(ddlPartFamily.SelectedValue.ToString());
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string reportName = "InspectionMaster_KTA.xlsx";
                ...
                if (data.Count > 0 && data != null)
                {
                    foreach (InspectionMasterData item in data)
                    {
                        ...
                        j++;
                        wsDts.Cells[row, j].Value = Convert.ToInt16(item.SortOrder);
                        row++;
                    }
                    row--;
        ...
        {
            List<string> CopyComplst = TMPTrakDataBase.GetAllComponentByPartFamily(ddlCopyPartFamily.SelectedValue.ToString());
        ...
        protected void ddlPartFamily_SelectedIndexChanged(object sender, EventArgs e)
        {
            BindComponentId();
        }

        protected void ddlCopyPartFamily_SelectedIndexChanged(object sender, EventArgs e)
        {
            ScriptManager.RegisterStartupScript(this, GetType(), "Copyviewmode", "CopyView();", true);
            GetComponentData();
        }
