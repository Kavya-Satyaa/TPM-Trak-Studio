    ...
    public partial class MachineUpDownTime : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    int.TryParse(Session["ScreenHeight"].ToString(), out int screenheight);
                    iniMachineIndex = (Convert.ToInt32(screenheight)) / 100;
                }
        ...
        public void getshiftTime()
        {
            ...
            try
            {
				...
				//Session["FromDate"] = Convert.ToDateTime(txtFromDate.Text).ToString("yyyy-MM-dd HH:mm:ss");
            }
            catch (Exception ex)
            {
        ...
        private void SquareWaveChartWithLinePDTDownTime(ref DateTime[] dataX0, ref DateTime[] dataXEndtime, ref Double[] dataY0, ref DateTime[] DNStart, ref DateTime[] DNEnd, ref DateTime[] dataX3, ref Double[] dataY3, double YPositionMachine, string machine)
        {
            try
            {
                ...
                if (OPDT != null)
                {
                    if (OPDT.ToString().ToUpper() == "Y")
                    {
                        ...
                        for (int k3 = 0; k3 < dvPDT.Count; k3++)
                        {
                            ...
                            dataX2[0] = DTStr[rec];
                            dataX2[1] = DTEtr[rec];
                            layer2 = c.addStepLineLayer(dataY1, 0x0101ff, machine); //Blue Color 0x0000ff
                ...
                for (int k1 = 0; k1 < dvICD.Count; k1++)
                {
                    ...
                    Ilayer2[rec] = c.addStepLineLayer(IdataY1, 0X800000, machine); //Magenta Color
        ...
        protected void AndonMode_CheckedChanged(object sender, EventArgs e)
        {
            if(AndonMode.Checked)
            {
