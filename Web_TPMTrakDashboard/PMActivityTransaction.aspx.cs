...
{
    public partial class PMActivityTransaction : System.Web.UI.Page
    {
        ...
        private void BindAtivityTransactionDetails()
        {
            try
            {
                ...
                DataTable dt = DataBaseAccess.getActivityTransactionData(ddlMachineId.SelectedValue, frequency, hdnCurrentDateTime.Value, hdnCurrentPrevNextParam.Value, frequencyValue);
        ...
        protected void gvPMTransactionDetails_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                ...
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    label.Text = (e.Row.DataItem as DataRowView).Row["Activity"].ToString();
                    ...
                    //linkBtn.ID = "lnkFileName";
                    //linkBtn.Text = (e.Row.DataItem as DataRowView).Row["FilePath"].ToString();
                    //linkBtn.OnClientClick = "return showSOPFiles(this);";
                    //e.Row.Cells[4].Controls.Add(linkBtn);
                    //hdn = new HiddenField();
                    //hdn.ID = "hdnFileID";
                    //hdn.Value = (e.Row.DataItem as DataRowView).Row["FileId"].ToString();
                    //e.Row.Cells[4].Controls.Add(hdn);

                    //DataTable dtRamrks = Session["PMTransactionRamrksData"] as DataTable;
                    //DataTable dtTargetTime = Session["PMTransactionTargetTimeData"] as DataTable;
                    int datatbleColStartCount = 5;
                    for (int col = 4; col < e.Row.Cells.Count; col++)
                    {
                        ...
                        //string headerName = gvActivityTransactionDetails.HeaderRow.Cells[col].Text;
                        //string remark = "", target = "";
                        //var remarkTargetRow = dtRamrks.AsEnumerable().Where(k => k.Field<string>("UniqueCode") == uniqueCode);
                        //if (remarkTargetRow != null)
                        //{
                        //    foreach (var item in remarkTargetRow)
                        //    {
                        //        remark = item[headerName].ToString();
                        //    }
                        //}
                        //remarkTargetRow = dtTargetTime.AsEnumerable().Where(k => k.Field<string>("UniqueCode") == uniqueCode);
                        //if (remarkTargetRow != null)
                        //{
                        //    foreach (var item in remarkTargetRow)
                        //    {
                        //        target = item[headerName].ToString();
                        //        if (!string.IsNullOrEmpty(target))
                        //        {
                        //            long timeinsecond = Convert.ToInt64(target);
                        //            int hr = (int)(timeinsecond / 3600);
                        //            int min = ((int)(timeinsecond / 60)) % 60;
                        //            target = hr.ToString() + "." + min.ToString();

                        //        }
                        //    }
                        //}
                        //label = new Label();
                        //label.CssClass = "lblPMRemarks";
                        //label.Text = remark;
                        //label.Style.Add("visibility", "hidden");
                        //e.Row.Cells[col].Controls.Add(label);

                        //label = new Label();
                        //label.CssClass = "lblPMTargetTime";
                        //label.Text = target;
                        //label.Style.Add("visibility", "hidden");
                        //e.Row.Cells[col].Controls.Add(label);
        ...
        [System.Web.Services.WebMethod(EnableSession = true)]
        public static bool saveActvityTransactionData(string activity, string frequency, string header, string machineid)
        {
            ...
            try
            {
                ...
                //}
                if (frequency == "Daily" || frequency == "Weekly" || frequency == "15 Days" || frequency == "1 Month" || frequency == "3 Month" || frequency == "6 Month")
                {
                    isUpdated = DataBaseAccess.saveActvityTransactionDetails(activity, frequency, Convert.ToDateTime(header).AddHours(6).ToString("yyyy-MM-dd HH:mm:ss"), DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), machineid);
                }
                else if (frequency == "1 Year" || frequency == "2 Year")
                {
                    isUpdated = DataBaseAccess.saveActvityTransactionDetails(activity, frequency, Convert.ToDateTime("Jan " + header).AddHours(6).ToString("yyyy-MM-dd HH:mm:ss"), DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), machineid);
                }
            }
            catch (Exception ex)
            { }
