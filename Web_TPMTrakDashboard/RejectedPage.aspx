<%@ Page Title="Rejection Information" Language="C#" MasterPageFile="~/Site_Master_2.Master" AutoEventWireup="true" CodeBehind="RejectedPage.aspx.cs" Inherits="Web_TPMTrakDashboard.RejectedPage" meta:resourcekey="PageResource1" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
        ...
        $(document).ready(function () {
            ...
            $('#btnToggal').change(function(){   
                ...
                if($('#btnToggal').prop("checked")==true){
                    ...
                    BindCategoryChart('RejectionCategory', 'column');
                    if($("#ddlCatagory").val()!="0"){
                        var category=$("#ddlCatagory").val();
                        BindCodeChart('RejectionCode','column',category);                       
                    }else{
                        BindCodeChart('RejectionCode','column','');
                    }
                    // BindCodeChart('RejectionCode', 'column');
                }else{
                      $.blockUI({ message: '<img runat="server" src="~/img/loadIcon/ajax-loader.gif" />' });
                    BindCategoryChart('RejectionCategory', 'line');
                    if($("#ddlCatagory").val()!="0"){
                        var category=$("#ddlCatagory").val();
                        BindCodeChart('RejectionCode','line',category);                       
                    }else{
                        BindCodeChart('RejectionCode','line','');
                    }
                    // BindCodeChart('RejectionCode', 'line');
                }
            });
            //-----------------End Toggle Condition----------------------
              $.blockUI({ message: '<img runat="server" src="~/img/loadIcon/ajax-loader.gif" />' });
            BindCategoryChart('RejectionCategory', '');
            function BindCategoryChart(param, type) {
                $.ajax({
                    type: "POST",
                    url: "RejectedPage.aspx/GetRejectedCategoryChart",
                    ...
                    chart: {
                        events: {
                            drillup: function (e) {
                                ...
                                //drillval--;
                                //BindRejectionchart(e.seriesOptions.data[0].beforeTitle,e.seriesOptions.name)
                                if(e.seriesOptions.data[0].beforeTitle == "")
                                {
                                    $("#ddlCatagory").val("0")
                                    if($("#ddlCatagory").val()=="0"){
                                        if($('#btnToggal').prop("checked")==true){
                            ...
                            drilldown: function (e) {
                                //xAxis[0].axisTitle.attr
                                //alert(this.series[0].data[e.point.x].name);
                                //if (xlabel == "") {
                                //    xlabel = data.XAxisTitle + ' - ' + this.series[0].data[e.point.x].name;
                                //}
                                
                                if(this.series[0].data[e.point.x].beforeTitle == "")
                                {
                                    drillval++;
                                    BindRejectionchart(this.series[0].data[e.point.x].afterTitel,this.series[0].data[e.point.x].name);
                                }
                                else
                                {
                                    BindRejectionchart(this.series[0].data[e.point.x].afterTitel,this.series[0].name);
                                }
                                chart.setTitle({
            ...
            function BindRejectionchart(GetYear,Category){
                $('#ddlCatagory').val(Category);
                $.ajax({
                    type: "POST",
                    url: "RejectedPage.aspx/GetRejectedCodeChartDrillval",
                    contentType: "application/json; charset=utf-8",
                    data: '{GetYear:"' + GetYear + '",Category:"' + Category + '"}',
                    dataType: "json",
                    success: function (response) {
                        var itmData = response.d;
                        LoadCodeChart(itmData, Category, 'RejectionCode');
                        $.unblockUI({});
                    },
                    error: function (jqXHR, textStatus, err) {
                        alert('Error: ' + err);
            ...
            BindCodeChart('RejectionCode','','');
            function BindCodeChart(param, type,category) {
                $.ajax({
                    type: "POST",
                    url: "RejectedPage.aspx/GetRejectedCodeChart",
                    contentType: "application/json; charset=utf-8",
            ...
            function BindDynamicShiftChart(month, day, type, color, param, catagory, code) {
                $.ajax({
                    type: "POST",
                    url: "RejectedPage.aspx/GetShiftChartData",
                    contentType: "application/json; charset=utf-8",
    ...
    <div class="row text-center" style="font-size: 18px; font-weight: 900; margin-bottom: 5px; color: white">
        <%=GetLocalResourceObject("RejectionInformation") %> - <%= Session["MachineId"]%>   &nbsp&nbsp;<input type="checkbox" id="btnToggal" data-toggle="toggle" data-on="<%=GetGlobalResourceObject("CommanResource","Bar") %>" data-off=" <%=GetGlobalResourceObject("CommanResource","Line") %>">
    </div>
