...
{
    public partial class RejectedPage : System.Web.UI.Page
    {
        public string chartType = string.Empty;
        static Random _r = new Random();
        static string strShift, plantId, strMonth, componentId, employeeId, strYear, strDay, machineId,CellID, view = string.Empty;
        ...
        public static Chart<Series> GetRejectedCategoryChart(string param, string type)
        {
            ...
            var chart = new Chart<Series>
            {
                Title = "TITLE",
                Subtitle = "SubTitle",
                XAxisTitle = HttpContext.GetLocalResourceObject("~/RejectedPage.aspx", "RejectionbyRejectionCategory").ToString(),
                    ...
                    foreach (var catagory in categoryData)
                    {
                        ...
                        foreach (DataRow drshift in results)
                        {
                            chart.drilldown[i].data.Add(new DrildownData
                            {
                                ...
                                beforeTitle = "Rejection by Rejection Category" + " : " + drshift["Catagory"].ToString() + " (" + strYear + "-" + drshift["Month"].ToString() + "-" + drshift["Day"].ToString() + ")"
                    ...
                    foreach (var catagory in categoryData)
                    {
                        ...
                        foreach (DataRow dr in results)
                        {
                            chart.drilldown[i].data.Add(new DrildownData
                            {
                                ...
                                beforeTitle = "Rejection by Rejection Category" + " : " + dr["Catagory"].ToString() + " (" + strYear + "-" + strMonth + ")"
                            });
                        ...
                        if (results != null)
                        {
                            foreach (DataRow drmonth in results)
                            {
                                chart.drilldown[i].data.Add(new DrildownData
                                {
                                    ...
                                    beforeTitle = "Rejection by Rejection Category" + " : " + drmonth["Catagory"].ToString() + " (" + strYear + ")"
                                });
                            }
                            i++;
                        }
                            ...
                            if (results != null)
                            {
                                foreach (DataRow dr in results)
                                {
                                    chart.drilldown[i].data.Add(new DrildownData
                                    {
                                        ...
                                        beforeTitle = "Rejection by Rejection Category" + " : " + dr["Catagory"].ToString() + " (" + strYear + "-" + month + ")"
                                    });
                                }
                                i++;
                            }
        ...
        public static Chart<Series> GetRejectedCodeChart(string param, string type, string category)
        {
            ...
            var chart = new Chart<Series>
            {
                Title = "TITLE",
                Subtitle = "SubTitle",
                XAxisTitle = HttpContext.GetLocalResourceObject("~/RejectedPage.aspx", "RejectionbyRejectionCode").ToString(),
                YAxisTitle = "YAxisTitle",
                YAxisTooltipValueSuffix = "YAxisTooltipValueSuffix"
            };
            if (type == "")
            ...
            {
                HttpContext.Current.Session["chartType"] = type;
            }
            DataTable dtYear = new DataTable();
            DataTable dtMonth = new DataTable();
            DataTable dtDay = new DataTable();
            ...
            try
            {
                ...
                if (strYear != "" && strMonth != "" && strDay != "")
                {
                    ...
                    foreach (var code in codeData)
                    {
                        ...
                        DataRow[] results = dtShift.Select("Month = " + strMonth + " AND Code ='" + code + "' AND Day ='" + strDay + "'");
                        foreach (DataRow drshift in results)
                        {
                            chart.drilldown[i].data.Add(new DrildownData
                            {
                                ...
                                beforeTitle = "Rejection by Rejection Code" + " : " + drshift["Code"].ToString() + " (" + strYear + "-" + drshift["Month"].ToString() + "-" + drshift["Day"].ToString() + ")"
                ...
                else if (strYear != "" && strMonth != "")
                {
                    ...
                    foreach (var code in codeData)
                    {
                        ...
                        DataRow[] results = dtDay.Select("Month = " + strMonth + " AND Code ='" + code + "'");
                        foreach (DataRow dr in results)
                        {
                            chart.drilldown[i].data.Add(new DrildownData
                            {
                                ...
                                beforeTitle = "Rejection by Rejection Code" + " : " + dr["Code"].ToString() + " (" + strYear + "-" + strMonth + ")"
                            });
                            monthData.Add(Convert.ToDateTime(dr["PDate"]).ToString("dd"));
                        }
                        i++;
                    ...
                    foreach (var code in codeData)
                    {
                        foreach (var day in monthData)
                        {
                            ...
                            DataRow[] results = dtShift.Select("Month = " + strMonth + " AND Code ='" + code + "' AND Day ='" + day + "'");
                            foreach (DataRow drshift in results)
                            {
                                chart.drilldown[i].data.Add(new DrildownData
                                {
                ...
                else if (strYear != "")
                {
                    ...
                    foreach (var code in codeData)
                    {
                        ...
                        DataRow[] results = dtMonth.Select("Code = '" + code + "'");
                        if (results != null)
                        {
                            foreach (DataRow drmonth in results)
                            {
                                chart.drilldown[i].data.Add(new DrildownData
                                {
                                    ...
                                    beforeTitle = "Rejection by Rejection Code" + " : " + drmonth["Code"].ToString() + " (" + strYear + ")"
                                });
                            }
                            i++;
                        }
                    ...
                    foreach (var code in codeData)
                    {
                        foreach (var month in monthName)
                        {
                            ...
                            DataRow[] results = dtDay.Select("Month = " + month + " AND Code ='" + code + "'");
                            if (results != null)
                            {
                                foreach (DataRow dr in results)
                                {
                                    chart.drilldown[i].data.Add(new DrildownData
                                    {
                                        ...
                                        beforeTitle = "Rejection by Rejection Code" + " : " + dr["Code"].ToString() + " (" + strYear + "-" + month + ")"
                                    });
                                }
                                i++;
                            }
        ...
        public static Chart<Series> GetShiftChartData(string strMonth, string strDay, string param, string type, string catagory, string code)
        {
            ...
            var chart = new Chart<Series>
            {
                Title = "TITLE",
                Subtitle = "SubTitle",
                XAxisTitle = (param == "RejectionCategory" ? HttpContext.GetLocalResourceObject("~/RejectedPage.aspx", "RejectionbyRejectionCategory").ToString() : HttpContext.GetLocalResourceObject("~/RejectedPage.aspx", "RejectionbyRejectionCode").ToString()) + " : " + xasis + " (" + strYear + "-" + strMonth + "-" + strDay + ")",
                YAxisTitle = "YAxisTitle",
                YAxisTooltipValueSuffix = "YAxisTooltipValueSuffix"
            };
            DataTable dtYear = new DataTable();
        ...
        [WebMethod]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public static Chart<Series> GetRejectedCodeChartDrillval(string GetYear, string Category)
        {
            string type = "";
            string date = string.Empty;
            var chart = new Chart<Series>
            ...
            if (employeeId.Equals("All", StringComparison.OrdinalIgnoreCase))
                employeeId = "";
            string[] nameandyear = GetYear.Split('(');
            string dateselected = nameandyear[1].TrimEnd(')');
            string[] yearmonthday = dateselected.Split('-');
            for(int k=0;k<yearmonthday.Length;k++)
            {
                switch(k)
                {
                    case 0:
                        strYear = yearmonthday[k];
                        strMonth = "";
                        strDay = "";
                        break; ;
                    case 1:
                        strMonth = yearmonthday[k];
                        strDay = "";
                        break; 
                    case 2:
                        strDay = yearmonthday[k];
                        break; 
                }
            ...
            try
            {
                ...
                if (strYear != "" && strMonth != "" && strDay != "")
                {
                    #region "Day Wise Information-------------"
                    date = strYear + "-" + strMonth + "-" + strDay;
                    dtMonth = BindCockpitView.BindRejRewDowntimeInfo(date, strShift, plantId, machineId, "currentday", "RejectionCode", componentId, CellID, employeeId, view);
                    DataRow[] catMonth = null;
                    if (Category == "")
                        catMonth = dtMonth.Select();
                    else
                    ...
                    chart.drilldown = new List<DrildownSeries>();
                    int i = 0;
                    dtShift = BindCockpitView.BindRejRewDowntimeInfo(date, strShift, plantId, machineId, "SHIFT", "RejectionCode", componentId, CellID, employeeId, view);
                    foreach (var code in codeData)
                    {
                        chart.drilldown.Add(new DrildownSeries
                        {
                ...
                else if (strYear != "" && strMonth != "")
                {
                    #region "month Wise Information ------------"
                    date = strYear + "-" + strMonth + "-" + "01";
                    dtMonth = BindCockpitView.BindRejRewDowntimeInfo(date, strShift, plantId, machineId, "currentMONTH", "RejectionCode", componentId, CellID, employeeId, view);
                    DataRow[] catMonth = null;
                    if (Category == "")
                        catMonth = dtMonth.Select();
                    else
                    ...
                    chart.drilldown = new List<DrildownSeries>();
                    int i = 0;
                    dtDay = BindCockpitView.BindRejRewDowntimeInfo(date, strShift, plantId, machineId, "DAY", "RejectionCode", componentId, CellID, employeeId, view);
                    foreach (var code in codeData)
                    {
                        chart.drilldown.Add(new DrildownSeries
                        {
                ...
                else if (strYear != "")
                {
                    #region "Year Wise Information -----------"
                    date = strYear + "-" + "01" + "-" + "01";
                    dtYear = BindCockpitView.BindRejRewDowntimeInfo(date, strShift, plantId, machineId, "YEAR", "RejectionCode", componentId, CellID, employeeId, view);
                    DataRow[] catYear = null;
                    if (Category != "")
                        catYear = dtYear.Select("Catagory = '" + Category + "'");
                    else
                    ...
                    dtMonth = BindCockpitView.BindRejRewDowntimeInfo(date, strShift, plantId, machineId, "MONTH", "RejectionCode", componentId, CellID, employeeId, view);
                    chart.drilldown = new List<DrildownSeries>();
                    int i = 0;
                    foreach (var code in codeData)
                    {
                    ...
                    //    {
                    //        date = strYear + "-" + month + "-" + "01";
                    //        dtDay = BindCockpitView.BindDashBoardGraph(date, strShift, plantId, machine, "DAY","RejectionCode");
                    //        foreach (DataRow day in dtDay.Rows)
                    //        {
                    //            chart.drilldown.Add(new DrildownSeries
                    //            {
                    ...
                    //            });
                    //            date = strYear + "-" + month + "-" + Convert.ToDateTime(day["PDate"]).ToString("dd");
                    //            dtShift = BindCockpitView.BindDashBoardGraph(date, strShift, plantId, machine, "SHIFT","RejectionCode");
                    //            foreach (DataRow drshift in dtShift.Rows)
                    //            {
                    //                chart.drilldown[i].data.Add(new DrildownData { name = drshift["Shift"].ToString(), y = Convert.ToDecimal(drshift["Parameter"].ToString()) });
                    //            }
