...
{
    public partial class MachineInformation_Shanthi : System.Web.UI.Page
    {
        ...
        [WebMethod]
        public static List<MachineInfoModel_Shanthi> MachineDetailsInfo()
        {
            List<MachineInfoModel_Shanthi> lstMachineInfo = null;
            try
            {
                lstMachineInfo = DownCodeInfoDataBase.GetAllMachineDetails_Shanthi("");
            }
            catch (Exception ex)
            {
                ErrorSignal.FromCurrentContext().Raise(ex);
        ...
        [WebMethod]
        public static string UpdateMachineInfo(MachineInfoModel_Shanthi model)
        {
            string msg = null;
            ...
            try
            {
                ...
                foreach (var item in model.ListMachineInfo)
                {
                    ...
                    {
                        DownCodeInfoDataBase.InsertUpdateMachineInformation_Shanthi(item.MachineID.Trim(), item.Description, item.InterfaceID, item.ProtcolInString, item.IP, item.PortNo, item.BulkDataTransferPortNo, item.mchrrate, TPMTrakEnabled, SmartTransEnabled, EthernetEnabled, Nto1Device, DNC, item.DNSIP, item.DNSIPPortNo, item.OPNNo, item.Process, item.RequestFolder, item.Responcefolder, item.ACKfolder, item.SPCfolder, out param);
