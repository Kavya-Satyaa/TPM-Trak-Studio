...
using Web_TPMTrakDashboard.Allied;
using Web_TPMTrakDashboard.Bajaj.Model;
using Web_TPMTrakDashboard.Models;

namespace Web_TPMTrakDashboard.VED
{
    public partial class AMMaster_VED : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                BindGrid();
                string val = ddlFrequency.SelectedValue.ToString();
                if (val == "Daily")
                {
                    duedate1.Visible = false;
                    duedate2.Visible = false;
                }
                else if(val=="Monthly" || val=="Weekly")
                {
                    duedate1.Visible = true;
                    duedate2.Visible = true; 
                }
            }
        }
        ...
        private void BindGrid()
        {
            try
            {
                List<AMMAsterData> list = AlliedDBAccess.GetAMMasterData(ddlMachineID.SelectedValue.ToString(), ddlFrequency.SelectedValue.ToString(), hdnrevID.Value != "" ? Convert.ToInt32(hdnrevID.Value) : 1, "VED");

                var sortedRecords = list.OrderBy(r =>Convert.ToInt32(r.CheckpointID));

                lvGrid.DataSource = sortedRecords;
                lvGrid.DataBind();

                if (ddlFrequency.SelectedValue == "Daily")
                {
                    bool dueDateVisibility = DetermineDueDateVisibility();
                    HtmlTableCell dueDateHeader = lvGrid.FindControl("duedate") as HtmlTableCell;

                    if (dueDateHeader != null)
                    {
                        dueDateHeader.Visible = dueDateVisibility;
                    }
                }
                else
                {
        ...
        private bool DetermineDueDateVisibility()
        {
            return false;
        }
        ...
        protected void linkeditRow_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else if (val == "Monthly" || val=="Weekly")
                {
                    duedate1.Visible = true;
                    duedate2.Visible = true;
                    txtDueDate.Text = (lvGrid.Items[rowIndex].FindControl("lblDueDate") as Label).Text;
                }
        ...
        protected void btnNew_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else if (ddlFrequency.SelectedValue.ToString().Equals("Monthly", StringComparison.OrdinalIgnoreCase))
                {
                    if (Convert.ToInt32(Session["MasterCount"].ToString()) >= 8)
                    {
                        HelperClass.openWarningToastrModal(this, "Can not add more than 8 Checkpoints");
