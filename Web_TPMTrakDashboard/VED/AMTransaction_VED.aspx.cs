...
{
    public partial class AMTransaction_VED : System.Web.UI.Page
    {
        ...
        static readonly string appPath = HttpContext.Current.Server.MapPath("~/VED");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                string Filename = "MaintenanceReport_VED.xlsx";
                string Source = GetReportPath(Filename);
                string Template = "MaintenanceReport_VED" + DateTime.Now + ".xlsx";
                string destination = Path.Combine(appPath, "GeneratedReports", SafeFileName(Template));
                if (!File.Exists(Source))
                {
                    Logger.WriteDebugLog("MaintenanceReport_VED template does not exists at - " + Source);
                }
                else
                {
                    int rowStart = 7;
